...
info = '''
	...
	!! at least one 802.1X wifi ssid needs to be available
...
def get_ssidNumbers(networkId):   
	...
	for n in network_ssids:
		# check for "authMode": "8021x-meraki" and "splashPage": "None"
		if ((n["authMode"] == "8021x-meraki") and (n["splashPage"] == "None")):
			ssids_dic[n['name']]=n['number']
	if (ssids_dic == {}):
		print('no 802.1X Wifi SSID available')
		sys.exit(0)
	print(ssids_dic)
	return(ssids_dic)
...
def create_1X_user(networkId, ssidNumber):     # Wifi 802.1X user
...
if __name__ == "__main__":
	...
	#
	new_user = create_1X_user(networkId, ssidNumber)
	#
