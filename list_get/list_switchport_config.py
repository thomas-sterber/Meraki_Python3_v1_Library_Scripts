...
info = '''
	list the switchport config of a switch

	Calls in organization.py, switch.py , networks.py , of Meraki Python library
...
def get_switchport(deviceSN):
	switchports = dashboard_call.switch.getDeviceSwitchPorts(deviceSN)
...
def print_switchports(switchports):
	for n in switchports:
		...
		print('Name '.rjust(adjust,' '), n['name'])
		print('Tags '.rjust(adjust,' '), n['tags'])
		print('PoE enabled '.rjust(adjust,' '), n['poeEnabled'])
		print('Type '.rjust(adjust,' '), n['type'])
		if (n['type'] == 'access'):
			print('Vlan '.rjust(adjust,' '), n['vlan'])
			print('Voice vlan'.rjust(adjust,' '), n['voiceVlan'])
		else:
			print('Allowed vlans'.rjust(adjust,' '), n['allowedVlans'])
		print('isolationEnabled'.rjust(adjust,' '), n['isolationEnabled'])
		print('rstpEnabled'.rjust(adjust,' '), n['rstpEnabled'])
		print('udld'.rjust(adjust,' '), n['udld'])
		print('portScheduleId'.rjust(adjust,' '), n['portScheduleId'])
		if ('accessPolicyType' in n): print('accessPolicyType'.rjust(adjust,' '), n['accessPolicyType'])
		if ('accessPolicyNumber' in n): print('accessPolicyNumber'.rjust(adjust,' '), n['accessPolicyNumber'])
	return()
